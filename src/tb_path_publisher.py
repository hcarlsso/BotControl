...
def new_goal_list_callback(data):
        '''
        This callback is called every time this node receives list of nodes from
        tb_rviz_interaction
        This publishes the first pose of the array to the Robot to navigate.
        There is slight difference in the message we receive from
        tb_rviz_interaction
	...
	goal_to_publish.header.stamp = rospy.get_rostime()
        ...
        # Set Frame type as map, indicating these are coordinates
        # with respect to map.
	...
	goal_to_publish.pose.position = data.poses[0].position
        ...
        # Currently we always publish the standard orientation for robot,
        # This can be
...
def goal_status_callback(data):
        '''
        This callback is called everytime this node receives the status message
        of Robot.
        The Status message is published frequently from node,
        always has the
        last goal
	...
	if len(data.status_list)!=0:
		currstatus = data.status_list[0].status
		if(currstatus == 1):
			goal_flag = True
		if((currstatus == 4 ) or (currstatus == 3)):
...
def start():
	...
	#Assign publisher that publishes the index of the goal just accomplished
	...
	#This keeps the function active till node are shutdown.
	rospy.spin()
